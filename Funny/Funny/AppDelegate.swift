//private 私有变量，只可以类内部访问 (属性和方法)
//fileprivate 私有变量 文件内部可以访问 （属性和方法）
//final 任何地方都不能override （方法）
//open 在framework外的范围外可以被继承 （主要用于class）也可用于方法
//public 在framework外的范围外不可以被继承
...
//MARK get
    ...
    func application(_ app: UIApplication, open url: URL, options: [UIApplicationOpenURLOptionsKey : Any] = [:]) -> Bool {
        if url.scheme == "YZFunny" {
            ...
            var tag = Int(subStrings[1]);
            let rootVC: ExtentNavigationViewController = self.window?.rootViewController as! ExtentNavigationViewController;
            let vc: RootViewController = rootVC.viewControllers.first as! RootViewController;
            ...
            if tag! >= 100 && tag! <= 102 {
                ...
                vc.present(QRStartScanViewController(isFromWindow: true), animated: true, completion: nil);
            }
