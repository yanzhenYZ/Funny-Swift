...
//MARK: - UI
    fileprivate func configUI() {
        ...
        itemBtn.setTitle("编辑", for: .normal);
        itemBtn.setTitleColor(FunnyManager.manager.color(0.0, G: 99.0, B: 251.0), for: .normal);
        itemBtn.addTarget(self, action: #selector(self.editButtonClick), for: .touchUpInside);
    ...
    func editButtonClick() {
        if itemBtn.titleLabel?.text == "编辑" {
            isEdit = true;
            itemBtn.setTitle("完成", for: .normal);
        }else{
            isEdit = false;
            itemBtn.setTitle("编辑", for: .normal);
        }
