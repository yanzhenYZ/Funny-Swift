...
class SecretNewItemViewController: UIViewController,SecretKeyBoardToolProtocol,SecretTitleViewDelegate {
    ...
    fileprivate func configUI() {
        ...
        let rowButton = UIButton(type: .system);
        rowButton.frame = CGRect(x: 30.0, y: rowTextField.y, width: WIDTH - 60, height: 50);
        rowButton.backgroundColor = UIColor.clear;
        rowButton.layer.masksToBounds = true;
        rowButton.layer.cornerRadius = 6.0;
        rowButton.addTarget(self, action: #selector(self.rowButtonAction), for: .touchUpInside);
        ...
        saveBtn.addTarget(self, action: #selector(self.saveButtonAction), for: .touchUpInside);
